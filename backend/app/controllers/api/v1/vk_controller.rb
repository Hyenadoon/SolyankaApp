  ...
  module V1
    class VkController < ApplicationController
      ...
      def launch
        ...
        unless vk_app_id_valid?(launch_params)
          return render json: { error: "Invalid VK app id" }, status: :unauthorized
        end
      ...
      def vk_app_id_valid?(launch_params)
        return true if configured_vk_app_id.blank?

        launch_params["vk_app_id"].to_s == configured_vk_app_id.to_s
      end

      def configured_vk_app_id
